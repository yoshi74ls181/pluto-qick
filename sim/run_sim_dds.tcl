...
create_project -force ddssim $work/proj -part xc7z020clg400-2
...
# The DDS Compiler derives Phase_Width from Frequency_Resolution in
# System_Parameters mode: w = ceil(log2(fs/res)). For w=32 the request must land
# in [fs/2^32, fs/2^31); aim at the middle of that window so it is not sensitive
# to rounding. QICK asks for 0.06 Hz, which is fine on an RFSoC where fs is ~40x
# higher, but at 122.88 MHz that is coarser than fs/2^31 and silently yields 31
# bits -- halving the phase accumulator and doubling every synthesised frequency.
set fs_hz [expr {122.88e6}]
set fres  [expr {$fs_hz * 1.5 / pow(2,32)}]
puts [format "DDS: fs=%.0f Hz -> requesting resolution %.4f Hz for a 32-bit phase" $fs_hz $fres]

set want [list \
   Parameter_Entry              System_Parameters \
   PartsPresent                 Phase_Generator_and_SIN_COS_LUT \
   DDS_Clock_Rate               122.88 \
   Frequency_Resolution         $fres \
   Phase_Width                  32 \
   Output_Width                 16 \
